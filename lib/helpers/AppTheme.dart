...
class AppTheme {
  ...
  static TextStyle getTextStyle(TextStyle? textStyle,
      ...
      double? fontSize}) {
    double finalFontSize = fontSize ?? textStyle?.fontSize ?? 14.0;

    Color finalColor;
    if (color == null) {
      finalColor = (xMuted
          ? textStyle?.color?.withAlpha(160) ?? Colors.black.withAlpha(160)
          : (muted ? textStyle?.color?.withAlpha(200) ?? Colors.black.withAlpha(200) : textStyle?.color ?? Colors.black));
    } else {
      finalColor = xMuted
          ? color.withAlpha(160)
